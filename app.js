...
app.get("/home", (req, res) => {
  const htmlContent = `
    ...
    <body>
      <div class="container">
        <img src="/img/logo.png" class="img-fluid mb-4" alt="Banner de Barbearia">
        <h1>Fila de Agendamentos</h1>
        <div id="agendamentos" class="mt-4"></div>
        <button class="btn btn-primary agendar" data-bs-toggle="modal" data-bs-target="#agendarModal">Agendar Horário</button>
      </div>
              ...
              <form id="formAgendamento">
                ...
                <div class="mb-3">
                  ...
                  <select class="form-select" id="servicoCliente" required>
                    ...
                    <option value="Corte com barba - R$ 40,00">Corte com barba - R$ 40,00</option>
                  </select>
                </div>
                <div class="mb-3">
                ...
                <div class="mb-3">
                  <label for="barbeiroCliente" class="form-label">Escolha o Barbeiro</label>
                  <select class="form-select" id="barbeiroCliente" required>
                    <option value="Jair Santana">Jair de Jesus</option>
                    <!-- Adicione mais barbeiros conforme necessário -->
                  </select>
                </div>
                <button type="submit" class="btn btn-primary confirma-agendamento">Confirmar agendamento</button>
              </form>
            </div>
          </div>
        </div>
...
io.on("connection", (socket) => {
  ...
  socket.on("agendarHorario", (data) => {
    ...
    console.log("Forma de Pagamento:", data.formaPagamento);  // Log da forma de pagamento
    console.log("Barbeiro:", data.barbeiro);  // Log do barbeiro
  
    // Verifica se algum dado está vazio
    if (!data.clienteId || !data.cliente || !data.horario || !data.servico || !data.formaPagamento || !data.barbeiro) {
    ...
    // Consulta para inserir o agendamento no banco de dados, incluindo o barbeiro
    const query = "INSERT INTO agendamentos (cliente, clienteId, horario, servico, forma_pagamento, barbeiro) VALUES (?, ?, ?, ?, ?, ?)";
    db.query(query, [data.cliente, data.clienteId, data.horario, data.servico, data.formaPagamento, data.barbeiro], (err, results) => {
      if (err) {
